...
MsOfficeWordHTMLWriter.prototype = {
    ...
    attach: function(filename, filepath, callback) {
        ...
        if (Buffer.isBuffer(filepath)) {
            self.MIME_parts.push(["files/" + filename, filepath]);
            if (callback) callback(self);
        } else {        
    ...
    content: function() {
        ...
        parts.forEach(function(pair) {
            ...
            if ( /^text|xml$/.test( mimeType ) ) {
                ...
                encoded  = mimelib.encodeQuotedPrintable( content.toString().replace(/\r\n/, "\n") );
            } else {
                encoding = 'base64';
                encoded  = Buffer.isBuffer(content) ? content.toString('base64') : mimelib.toString().encodeBase64(content);
            }
